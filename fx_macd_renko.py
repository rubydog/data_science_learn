...
def MACD(DF, a, b, c):
    ...
    df = DF.copy()
    df["MA_Fast"]=df["Close"].ewm(com=a,min_periods=a).mean()
...
def renko_DF(DF):
    ...
    renko_df["bar_num"] = np.where(renko_df["uptrend"]==True, 1, -1) #np.where(renko_df["uptrend"]==False,-1,0))
        ...
        for currency in pairs:
            ...
            if signal == "Buy":
                con.open_trade(symbol=currency, is_buy=True, is_in_pips=True, amount=pos_size,
                               time_in_force='GTC', stop=-8, order_type='AtMarket')
...
while time.time() <= timeout:
    try:
        ...
        print("Keyboard execution received. Exiting.")
        exit()
